...
from linear_attention_bigbird_wrapper import BigBirdForSequenceClassification
...
def get_model(config, model_config):
    ...
    # Create model
    model = BigBirdForSequenceClassification(model_config)
...
def train(model, config, use_deepspeed,early_stopper):
    ...
    writer = SummaryWriter("runs/linear_mini_lra_listops")
    ...
    eval_best_acc = 0
    eval_best_counter = 0
    threshold_for_early_stop = 0.00001
    threshold_window = 3
    ...
    for i, (inputs, target) in enumerate(pbar):
        ...
        if (config.eval_frequency > 0) and ((i+1) % config.eval_frequency == 0):
            ...
            average_eval_loss = 0.
            average_eval_acc = 0.
            eval_dataloader = DataLoader(eval_dataset, batch_size=batch_size//8, 
