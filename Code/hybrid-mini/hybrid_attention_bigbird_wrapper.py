# Code was obtained from https://github.com/huggingface/transformers/blob/main/src/transformers/models/big_bird/configuration_big_bird.py
# Hugging Face. (2023). transformers/src/transformers/models/big_bird/configuration_big_bird.py. GitHub. 
# https://github.com/huggingface/transformers/blob/main/src/transformers/models/big_bird/configuration_big_bird.py
...
class LinearAttention(nn.Module):
    def __init__(self, config, seed=None):
        ...
        # BigBird uses these exact names in its attention implementations
    ...
    def feature_map(self, x):
        """Applies ELU feature map to approximate softmax."""
    ...
    def forward(
        ...
        # These are ignored but kept for compatibility
        band_mask=None,
        from_mask=None,
        to_mask=None,
        ...
        if encoder_hidden_states is not None:
            raise ValueError("LinearAttention doesn't support cross-attention")
            
        batch_size, seq_len, _ = hidden_states.shape

        # Compute Q, K, V - using same names as BigBird
        ...
        # Compute attention
        kv = torch.einsum("bnhd,bnhm->bhdm", k, v)  # (B, H, D, D)
        z = 1 / (torch.einsum("bnhd,bhd->bnh", q, k.sum(dim=1)) + 1e-6)  # (B, N, H)
        
        attention_output = torch.einsum("bnhd,bhdm->bnhm", q, kv)  # (B, N, H, D)
        attention_output = attention_output * z.unsqueeze(-1)
...
class BigBirdLayer(nn.Module):
    def __init__(self, config, seed=None, layer_idx=0):
        super().__init__()
        self.config = config
        self.attention_type = config.attention_type
        ...
        # self.attention = BigBirdAttention(config, seed=seed)
         # Alternate between LinearAttention and BigBirdAttention
        if layer_idx %2 == 0:  # Even layers use LinearAttention
            self.attention = LinearAttention(config, seed=seed)
        else:  # Odd layers use BigBirdAttention
            self.attention = BigBirdAttention(config, seed=seed)
        self.is_decoder = config.is_decoder
    ...
    def set_attention_type(self, value: str):
        ...
        # self.attention.set_attention_type(value)
         # Only modify the BigBirdAttention layers, leave LinearAttention as is
        if isinstance(self.attention, BigBirdAttention):
            self.attention.set_attention_type(value)
...
class BigBirdEncoder(nn.Module):
    def __init__(self, config):
        ...
        # )
        self.layer = nn.ModuleList(
            [BigBirdLayer(config, seed=layer_idx, layer_idx=layer_idx) 
